...
* doc). `direction` below refers to `game.util.ArrowSplitDirection`, not
* Flixel's `FlxDirectionFlags` - see the note in ArrowSplitDirection.hx about the two.
 */
class ArrowBroken extends FlxSprite
{
	...
	public function initialize(direction:Int, X:Float, Y:Float):Void
	{
		...
		var sign:Int = (direction == ArrowSplitDirection.LEFT) ? -1 : 1;
